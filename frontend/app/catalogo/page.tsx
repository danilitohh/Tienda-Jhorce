...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  return <><SiteHeader /><main className="site-shell min-h-[65vh] py-14 sm:py-20 lg:py-24"><div className="max-w-2xl"><p className="eyebrow">La tienda byjhor</p><h1 className="mt-5 font-display text-6xl font-semibold leading-[.9] tracking-[-.04em] sm:text-8xl">Encuentra tu<br />próxima peluca.</h1><p className="mt-6 max-w-lg text-base leading-7 text-muted">Explora pelucas para cada día, estilos con movimiento y accesorios para cuidarlas.</p></div><div className="mt-14"><CatalogView key={initialCategory} products={listProducts()} initialCategory={initialCategory} /></div></main><SiteFooter /></>;
}
