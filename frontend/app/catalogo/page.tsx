...
import { listProducts, type ProductCategory } from "@backend/catalog/catalog-data";
...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const heroProduct = products.find((product) => selectedCategory === "Todos" ? product.category === "Movimiento" : product.category === selectedCategory) ?? products[0];
