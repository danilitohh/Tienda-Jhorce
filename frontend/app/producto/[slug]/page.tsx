...
import { getStoreProductBySlug, loadStoreCatalog } from "@/lib/catalog-repository";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const product = await getStoreProductBySlug((await params).slug);
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = (await loadStoreCatalog()).filter((item) => item.status === "ACTIVE" && item.id !== product.id && item.category === product.category).slice(0, 3);
  return <><SiteHeader /><main className="site-shell py-8 sm:py-10 lg:py-12"><div className="mb-10 flex flex-wrap items-center gap-2 text-xs text-muted"><Link href="/catalogo" className="transition-colors hover:text-ink">Tienda</Link><CaretRight size={14} /><span>{getCategoryLabel(product.category)}</span><CaretRight size={14} /><span className="text-ink">{product.name}</span></div><ProductDetails product={product} /><section className="mt-20 border-t border-ink/10 pt-14"><div className="flex items-end justify-between"><div><p className="eyebrow">Sigue explorando</p><h2 className="mt-3 font-display text-4xl font-semibold leading-none tracking-[-.03em]">También puede gustarte</h2></div><Link href="/catalogo" className="hidden text-sm font-semibold transition-colors hover:text-accent-deep sm:block">Ver todo</Link></div><div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:gap-6">{related.map((item) => <ProductCard key={item.id} product={item} />)}</div></section></main><SiteFooter /></>;
}
