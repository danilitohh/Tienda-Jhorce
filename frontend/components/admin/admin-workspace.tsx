...
type ApiResponse = Readonly<{ data?: { product?: AdminProduct; order?: AdminOrder; products?: AdminProduct[]; imported?: number }; error?: { message?: string } }>;
...
export function AdminWorkspace({ data }: Readonly<{ data: AdminDashboardData }>) {
  ...
  // Request the protected bootstrap action and replace the local table with the server's canonical Mongo projection.
  const importCatalog = async () => {
    setImporting(true);
    setNotice(null);
    try {
      const response = await fetch("/api/admin/catalog/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}", cache: "no-store" });
      const result = await response.json() as ApiResponse;
      if (!response.ok || !result.data?.products) { setNotice(result.error?.message ?? "No pudimos cargar el catálogo actual."); setImporting(false); return; }
      setProducts(result.data.products);
      setNotice(result.data.imported ? `Se importaron ${result.data.imported} productos al catálogo.` : "El catálogo actual ya estaba cargado.");
    } catch { setNotice("No pudimos conectar con MongoDB. Inténtalo de nuevo."); }
    setImporting(false);
  };
  ...
  return <>
    ...
    {tab === "catalog" && <section className="mt-8" aria-labelledby="catalog-title">{formOpen ? <AdminProductForm product={editingProduct} onSaved={handleSaved} onCancel={() => { setFormOpen(false); setEditingProduct(null); }} /> : <><div className="flex flex-col justify-between gap-5 border-b border-ink/15 pb-5 sm:flex-row sm:items-end"><div><p className="eyebrow">Gestión de productos</p><h2 id="catalog-title" className="mt-2 font-display text-4xl font-semibold leading-none">Tu catálogo, siempre al día.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-muted">Crea productos, ajusta precios y stock, y decide qué queda visible para tus clientes.</p></div><div className="flex flex-wrap gap-3"><button type="button" onClick={importCatalog} disabled={importing} className="button-secondary shrink-0"><Storefront size={17} /> {importing ? "Cargando catálogo..." : "Cargar catálogo actual"}</button><button type="button" onClick={openCreate} className="button-primary shrink-0"><Plus size={17} /> Nuevo producto</button></div></div><label className="relative mt-5 block max-w-md"><span className="sr-only">Buscar en el catálogo</span><MagnifyingGlass className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="field-input w-full pl-10" placeholder="Buscar por nombre o slug" /></label><ProductTable products={visibleProducts} onEdit={openEdit} emptyMessage={query ? "No encontramos productos con esa búsqueda." : "Todavía no hay productos. Usa «Cargar catálogo actual» para llevar aquí los productos de la tienda, o crea uno nuevo."} /></>}</section>}
