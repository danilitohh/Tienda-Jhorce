    ...
    <div className="rounded-[10px] border border-ink/10 bg-white p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div id="catalog-filters" className={`${filtersOpen ? "grid" : "hidden"} gap-4 lg:flex lg:items-center`}>
          <div className="flex flex-wrap gap-2">{categories.map((item) => <button key={item} onClick={() => setCategory(item)} aria-pressed={category === item} className={`rounded-[8px] border px-3 py-2 text-xs font-medium transition-colors ${category === item ? "border-ink bg-ink text-white" : "border-ink/15 bg-transparent text-ink hover:border-ink"}`}>{item}</button>)}</div>
