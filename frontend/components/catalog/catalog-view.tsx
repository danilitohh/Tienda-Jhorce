  ...
  return <>
    <div className="rounded-[10px] border border-ink/10 bg-white p-4 sm:p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative flex-1 lg:max-w-sm"><StoreSearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="field-input w-full pl-10 pr-9" placeholder="Buscar por nombre o categoría" aria-label="Buscar productos" />{query && <button onClick={() => setQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted transition-colors hover:text-ink" aria-label="Limpiar búsqueda"><X size={16} /></button>}</div>
        ...
        <div id="catalog-filters" className={`${filtersOpen ? "grid" : "hidden"} gap-4 lg:flex lg:items-center`}>
          <div className="flex flex-wrap gap-2">{categories.map((item) => <button key={item.value} onClick={() => setCategory(item.value)} aria-pressed={category === item.value} className={`rounded-[8px] border px-3 py-2 text-xs font-medium transition-colors ${category === item.value ? "border-teal bg-teal text-white" : "border-ink/15 bg-transparent text-ink hover:border-teal hover:bg-sage/40"}`}>{item.label}</button>)}</div>
          <label className="flex items-center gap-2 border-t border-ink/10 pt-3 text-xs font-medium lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0"><SlidersHorizontal size={16} /><span className="sr-only">Ordenar</span><select value={sort} onChange={(event) => setSort(event.target.value)} className="rounded-[8px] border border-ink/15 bg-paper px-3 py-2 text-xs outline-none focus:border-teal"><option value="featured">Destacados</option><option value="price-asc">Precio menor</option><option value="price-desc">Precio mayor</option></select></label>
        </div>
      </div>
    </div>
