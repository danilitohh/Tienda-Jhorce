...
export function ProductDetails({ product }: Readonly<{ product: StoreProduct }>) {
  ...
  return <div className="grid gap-10 lg:grid-cols-[1.15fr_.85fr] lg:gap-16">
    <div className="grid gap-3 sm:grid-cols-[90px_1fr]">
      ...
      <div className="relative order-1 aspect-[4/5] overflow-hidden rounded-[10px] bg-sand sm:order-2"><Image src={image} alt={product.name} fill priority sizes="(max-width: 768px) 100vw, 55vw" className="object-cover" /></div>
    </div>
    <div className="lg:py-8"><div className="flex items-center gap-2 text-xs uppercase tracking-[.18em] text-muted"><span>{product.category}</span><span aria-hidden="true">/</span><span>byjhor</span></div><h1 className="mt-4 font-display text-5xl font-semibold leading-[.92] tracking-[-.03em] sm:text-6xl">{product.name}</h1><div className="mt-5 flex items-center gap-3"><span className="font-body text-lg font-semibold tabular-nums">{formatCop(product.price)}</span>{product.compareAtPrice && <span className="text-sm text-muted line-through">{formatCop(product.compareAtPrice)}</span>}<span className="ml-auto flex items-center gap-1 text-sm"><Star size={16} weight="fill" className="text-accent" /> {product.rating} <span className="text-muted">({product.reviewCount})</span></span></div><p className="mt-6 max-w-md text-[15px] leading-7 text-muted">{product.description}</p>
      {product.sizes && <div className="mt-9"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[.16em]">Talla</p><span className="text-xs text-muted">Selecciona una opción</span></div><div className="flex flex-wrap gap-2">{product.sizes.map((size) => <button key={size} onClick={() => setSelectedSize(size)} aria-pressed={selectedSize === size} className={`min-w-12 rounded-[8px] border px-4 py-3 text-sm transition-colors ${selectedSize === size ? "border-ink bg-ink text-white" : "border-ink/15 hover:border-ink"}`}>{size}</button>)}</div></div>}
      {product.colors && <div className="mt-7"><p className="mb-3 text-xs font-bold uppercase tracking-[.16em]">Color <span className="font-normal normal-case tracking-normal text-muted">{selectedColor}</span></p><div className="flex flex-wrap gap-2">{product.colors.map((color, index) => <button key={color} onClick={() => setSelectedColor(color)} aria-pressed={selectedColor === color} className={`flex items-center gap-2 rounded-[8px] border px-3 py-2 text-xs transition-colors ${selectedColor === color ? "border-ink" : "border-transparent hover:border-ink/30"}`}><span className={`h-5 w-5 rounded-full border border-ink/10 ${index === 0 ? "bg-ink" : "bg-sand"}`} />{color}</button>)}</div></div>}
