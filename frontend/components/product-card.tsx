...
export function ProductCard({ product }: Readonly<{ product: StoreProduct }>) {
  ...
  return (
    <article className="group flex h-full flex-col">
      ...
      <div className="flex min-h-[7.8rem] flex-1 items-end justify-between gap-3 pt-3 sm:min-h-[8.6rem] sm:pt-4">
        ...
        <div className="flex shrink-0 flex-col items-end gap-3 self-end">
          ...
          {hasOptions ? <Link href={`/producto/${product.slug}`} className="card-action" aria-label={`Elegir opciones de ${product.name}`}><ArrowUpRight size={17} /><span className="sr-only">Elegir opciones</span></Link> : <button onClick={() => addItem(product)} className="card-action" aria-label={`Añadir ${product.name} al carrito`}><Plus size={18} /></button>}
        </div>
      </div>
    </article>
  );
