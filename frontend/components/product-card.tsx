...
export function ProductCard({ product }: Readonly<{ product: StoreProduct }>) {
  ...
  return <article className="group h-full">
    ...
    <div className="flex items-start justify-between gap-3 pt-4">
      <div><p className="text-[0.65rem] font-bold uppercase tracking-[.16em] text-accent-deep">{getCategoryLabel(product.category)}</p><Link href={`/producto/${product.slug}`} className="mt-2 block font-display text-[1.35rem] font-semibold leading-none tracking-[-.02em] transition-colors hover:text-accent-deep">{product.name}</Link><p className="mt-2 text-xs text-muted">{product.colors?.length ? `${getColorLabel(product.category)}: ${product.colors[0]}` : "Accesorio para cuidado"}</p>{product.badge && <p className="mt-2 text-xs font-medium text-accent-deep">{product.badge}</p>}</div>
