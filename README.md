...
- MongoDB admin workspace with a separate private username/password session, product CRUD, inventory editing, order status operations and GridFS image uploads, plus a legacy Prisma planning schema for future relational imports;
...
- The catalog repository reads published products from MongoDB and falls back to the current local catalog only while the configured products collection is empty. When the owner opens `/admin` and the manageable products collection is empty, the server automatically imports the six current products without overwriting existing records; the bootstrap preserves IDs and slugs and is idempotent. Products created or edited in `/admin` use the same public product contract and become visible in the storefront after publication.
- Product images selected in the admin form are stored in MongoDB GridFS through the protected `/api/admin/media` route. Configure `MONGODB_MEDIA_BUCKET` only if the default `media` bucket must be changed; each uploaded JPG, PNG or WebP is limited to 4 MB and receives a public immutable `/api/media/:id` URL.
