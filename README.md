...
- The catalog repository reads published products from MongoDB and falls back to the current local catalog only while the configured products collection is empty. Products created or edited in `/admin` use the same public product contract and become visible in the storefront after publication. If the `products` collection is empty, open `/admin` > `Catálogo` > `Cargar catálogo actual` to import the six current products without overwriting existing records; the action preserves IDs and slugs and is idempotent.
